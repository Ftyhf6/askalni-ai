...
function App() {
  ...
  const characterText = customCharacter.trim() || selectedCharacter?.description || "لم يتم اختيار شخصية بعد";
      ...
      <div className="mx-auto max-w-5xl px-4 py-5 sm:px-6">
        ...
        {step === "idea" && (
          <section className="rounded-3xl border border-white/10 bg-white/[0.06] p-5 shadow-2xl">
            <div className="mb-5">
              ...
              <p className="mt-2 text-slate-400">اكتب فكرتك بطريقتك الطبيعية. اسألني سيحللها ثم يسألك فقط عن التفاصيل الضرورية الناقصة.</p>
            </div>
            <textarea
              value={idea}
            ...
            />
            <button onClick={analyzeIdea} className="mt-4 w-full rounded-2xl bg-amber-400 px-5 py-3 font-bold text-slate-950 hover:bg-amber-300">
          ...
          <section className="space-y-5">
            <div className="rounded-3xl border border-white/10 bg-white/[0.06] p-5">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {["الكل", ...categories].map((item) => (
                  <button key={item} onClick={() => setFilter(item)} className={`rounded-full px-3 py-1.5 text-sm ${filter === item ? "bg-amber-400 text-slate-950" : "bg-white/10 text-slate-300"}`}>
                    {item}
                  </button>
                ))}
              </div>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
                {visibleCharacters.map((character) => (
                  <button key={character.id} onClick={() => { setSelectedCharacter(character); setCustomCharacter(""); }} className={`rounded-2xl border p-4 text-right transition ${selectedCharacter?.id === character.id ? "border-amber-400 bg-amber-400/10" : "border-white/10 bg-slate-900 hover:border-white/25"}`}>
              ...
              <input value={customCharacter} onChange={(e) => setCustomCharacter(e.target.value)} placeholder="أو اكتب وصف الشخصية التي تريدها..." className="mt-4 w-full rounded-2xl border border-white/10 bg-slate-900 px-4 py-3 outline-none focus:border-amber-400" />

              <label className="mt-5 block text-sm font-semibold">النمط البصري</label>
              <div className="mt-2 flex flex-wrap gap-2">
                {styles.map((item) => (
                  <button key={item} onClick={() => setStyle(item)} className={`rounded-xl border px-3 py-2 text-sm ${style === item ? "border-amber-400 bg-amber-400/10 text-amber-200" : "border-white/10 bg-slate-900 text-slate-300"}`}>
                    {item}
                  </button>
                ))}
              </div>
            ...
            <div className="rounded-3xl border border-white/10 bg-white/[0.06] p-5">
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-3">
                <label className="rounded-2xl bg-slate-900 p-4">
                  ...
                  <input type="number" min={1} max={60} value={duration} onChange={(e) => setDuration(Math.min(60, Math.max(1, Number(e.target.value) || 1)))} className="mt-2 w-full bg-transparent text-2xl font-bold outline-none" />
                </label>
                <label className="rounded-2xl bg-slate-900 p-4">
                  <span className="text-sm text-slate-400">اللغة</span>
                  <select value={language} onChange={(e) => setLanguage(e.target.value)} className="mt-2 w-full bg-transparent font-bold outline-none">
                    <option>العربية</option><option>English</option><option>اردو</option><option>کوردی</option>
                  </select>
                </label>
            ...
            <div className="rounded-3xl border border-amber-400/30 bg-amber-400/10 p-5">
              ...
              <div className="mt-4 flex flex-col gap-3 sm:flex-row">
                <button onClick={() => setStep("confirm")} className="flex-1 rounded-2xl bg-amber-400 px-5 py-3 font-bold text-slate-950">اكتمال التفاصيل → مراجعة التنفيذ</button>
                <button onClick={() => setStep("idea")} className="rounded-2xl border border-white/10 px-5 py-3">تعديل الفكرة</button>
              </div>
            </div>
          </section>
        )}
        ...
        {step === "confirm" && (
          <section className="rounded-3xl border border-white/10 bg-white/[0.06] p-6">
            ...
            <button onClick={confirmPlan} className="mt-5 w-full rounded-2xl bg-emerald-400 px-5 py-4 font-bold text-slate-950">ابدأ التنفيذ</button>
        ...
        {step === "done" && (
          <section className="rounded-3xl border border-emerald-400/30 bg-emerald-400/10 p-7 text-center">
            ...
            <p className="mt-2 text-slate-300">هذه الواجهة أصبحت جاهزة لربط مرحلة التنفيذ الفعلية بخدمة الذكاء الاصطناعي وتوليد المشاهد والفيديو.</p>
            <button onClick={() => setStep("idea")} className="mt-5 rounded-2xl bg-white px-5 py-3 font-bold text-slate-950">إنشاء قصة جديدة</button>
          </section>
        )}
